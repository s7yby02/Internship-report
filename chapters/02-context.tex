...
\section{The Data Platform team and the working method: the Qonto Way}
...
\caption{The Data Platform Delivery Board during the internship: ongoing projects arranged by lifecycle stage (Value Analysis $\rightarrow$ Value Engineering $\rightarrow$ Build $\rightarrow$ Value Validation), each with a progress percentage, its blockers and a target date. The CESOP project (Build column) is assigned to the author.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{imgs/job_orchestrator.png}
\caption{Two execution models. In the monorepo, all DAGs share one Docker base image, so a single library bump can put every pipeline at risk. With the Job Orchestrator, a job is declared in \texttt{schedule.yaml} and its DAG is auto-generated, each task running in an isolated Kubernetes pod carrying the job's own image and dependencies.}
\label{fig:job-orch}
\end{figure}
...
\begin{lstlisting}[language=yaml,caption={Condensed Qontract for the Notion \texttt{data\_bugs} table (real contract, abridged).},label={lst:qontract}]
apiVersion: 'v3.1.0'
kind: 'DataContract'
...
schema:
  - name: 'data_bugs'
    logicalType: 'object'          # -> a Snowflake table
    physicalType: 'table'
    properties:
...
customProperties:
  ...
  - property: 'ingestion'
    value:
      page_size: 50
      resource_id: '2e7b3f6c73bc4da19fef375e932d20e1'   # which Notion data source to read
\end{lstlisting}
