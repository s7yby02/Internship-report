...
The team applies a structured method, internally called the \emph{Qonto Way}, which distinguishes two phases in a project: \emph{Value Analysis} (VA) where we frame the problem, stakeholders, the definition of ``done'' and the possible approaches then \emph{Value Engineering} (VE) with the detailed design, resolution of the sticking points (\emph{Hard Points}) and construction. Each project is documented on a Notion space following this template. I followed this approach for all three projects, which strongly structured my work: identify the true cause before coding, make trade-offs explicit, and list the sticking points to resolve.
