...
\begin{lstlisting}[language=SQL,caption={The \texttt{RAW.CESOP\_NL.SUBMISSIONS} state table (\texttt{jobs/cesop\_nl/state/schemas.py}).},label={lst:submissions}]
...
\begin{itemize}
  ...
  \item \textbf{submit/}: DAG~A's flow (fetch, validate, digest, \texttt{.meta}, upload, record).
...
\begin{lstlisting}[language=XML,caption={The Digipoort \texttt{.meta} sidecar built for a payload (structure fixed by \texttt{metabestand.xsd}).},label={lst:meta}]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{imgs/cesop_slack_notif.png}
